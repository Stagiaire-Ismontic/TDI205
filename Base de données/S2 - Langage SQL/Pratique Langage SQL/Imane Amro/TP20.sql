create procedure NombreJours1(@id_tache int)
as
begin
	...
	ELSE
	begin
		update taches set nombre_jours_realisation = (select avg(datediff(day,date_debut,date_fin)) from TachesStagiairees where id=@id_tache) where taches.id=@id_tache;
	end
end
...
--question 2
create procedure NombreJours(@id_projet int)
as
begin 
