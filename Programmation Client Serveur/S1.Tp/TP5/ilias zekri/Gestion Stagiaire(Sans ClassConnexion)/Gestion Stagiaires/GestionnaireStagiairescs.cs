...
namespace Gestion_Stagiaires
{
    ...
    {
        SqlCommand Cmd;
        string Chaine= @"Data Source =DESKTOP-PDQA7KA; Initial Catalog = GestionStagiaires; Integrated Security = True";
        ...
        public Stagiaire Recherche(int id)
        {
            ...
            if (Rd.HasRows)
            {
                Stagiaire S = new Stagiaire(Rd.GetInt32(0), Rd.GetString(1), Rd.GetString(2),Rd.GetString(3));
                return S;
            }
            return null;
        }
        ...
        public bool Ajouter(Stagiaire St)
        {
            using (SqlConnection Cnx = new SqlConnection(Chaine))
            {
                ...
                {
                    Cnx.Open();
                    Cmd = new SqlCommand("Insert into Stagiaire(Id,Cin,Nom,Groupe) values(" + St.Id + ",'" + St.Cin + "','" + St.Nom + "','" + St.Groupe + "')", Cnx);
                    Cmd.ExecuteNonQuery();
                    return true;
                }
                return false;
        ...
        public bool Modifier(Stagiaire Sta)
        {
            ...
            using (SqlConnection Cnx = new SqlConnection(Chaine))
            {
                ...
                {
                    Cnx.Open();
                    Cmd = new SqlCommand("Update Stagiaire set Nom= '" + Sta.Nom + "',Cin='" + Sta.Cin + "',DateT=',Groupe='" + Sta.Groupe + "' where Id=" + Sta.Id, Cnx);
                    Cmd.ExecuteNonQuery();
                    return true;
                }
                return false;
        ...
        public List<Stagiaire> Afficher()
        {
            using (SqlConnection Cnx = new SqlConnection(Chaine))
            {
                ...
                while (Rd.Read())
                {
                    LstStagiaires.Add(new Stagiaire(Rd.GetInt32(0), Rd.GetString(1), Rd.GetString(2), Rd.GetString(3)));
                }
