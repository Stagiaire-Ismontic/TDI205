    ...
    public class GestionFac
    {
        ...
        public Facture Recherche(int id)
        {
            ...
            if (Rd.HasRows)
            {
                while (Rd.Read())
                {
                   F = new Facture(Rd.GetInt32(0), Rd.GetString(1), double.Parse(Rd.GetDecimal(2).ToString()), new Client() { Id = Rd.GetInt32(3) });
                }
                return F;
            }
            return null;
        ...
        public bool Ajouter(Facture St)
        {
            ...
            if (F == null)
            {
                ...
                string S = "Insert into Facture values(" + St.Id + ",'" + St.Titre + "'," + St.Somme + "," + St.Client.Id + ")";
                C.ExecuteR(S);
                return true;
            }
            return false;
        ...
        public bool Modifier(Facture St)
        {
            ...
            if (F != null)
            {
                string S = "Update Facture set Titre= '" + St.Titre + "',Somme=" + St.Somme + ",ClientId=" + St.Client.Id + " where IdFacture=" + St.Id;
                C.ExecuteR(S);
                return true;
            }
            return false;
